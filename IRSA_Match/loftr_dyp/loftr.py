...
class LoFTR(nn.Module):
    ...
    def getfeature(self, img=None, feat_c0=None, feat_f0=None):
        if feat_c0 is None:
            feat_c0, feat_f0 = self.backbone(img)
        feat_c0 = self.pos_encoding(feat_c0)
        c0sp = feat_c0.shape[2:]
        feat_c0 = rearrange(feat_c0, 'n c h w -> n (h w) c')
        if feat_f0 is None:
            return feat_c0, None, c0sp,  None
        else:
            return feat_c0, feat_f0, c0sp,  feat_f0.shape[2:]
    
    
    def coarse_match(self, feat_c0, feat_c1, data):
        # start_t = time.time()
        feat_c0, feat_c1 = self.loftr_coarse(feat_c0, feat_c1, None, None)
        self.coarse_matching(feat_c0, feat_c1, data)
        # print("match 2  ", time.time() - start_t)
        # start_t = time.time()
        # 3. match coarse-level
        # print(feat_c0.shape, feat_c1.shape)
        
        # print(feat_c0.shape, feat_c1.shape)
        # print("match 3  ", time.time() - start_t)


    def fine_match(self, feat_c0, feat_f0, feat_c1, feat_f1, data):
        # start_t = time.time()
        ...
        # print("match 4  ", time.time() - start_t)
        # start_t = time.time()
        # 5. match fine-level
        self.fine_matching(feat_f0_unfold, feat_f1_unfold, data)

        # print("match  5 ", time.time() - start_t)
        # start_t = time.time()
    ...
    def forward(self, data):
        ...
        print("backbone   ", time.time() - start_t)
        start_t = time.time()
